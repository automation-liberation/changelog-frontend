...
volumes: [
  hostPathVolume(mountPath: '/var/run/docker.sock', hostPath: '/var/run/docker.sock')
    ...
    node(label) {
        stage('Build') {
            git 'https://github.com/automation-liberation/changelog-frontend.git'
            script {
        ...
        stage('Deploy') {
            container('helm') {
                dir('charts') {
                    sh "helm upgrade changelog-frontend changelog-frontend --install --set image.tag=${properties.image.tag}"
                }
            }
        }
